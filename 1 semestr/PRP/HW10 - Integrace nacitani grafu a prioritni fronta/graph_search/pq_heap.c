...
_Bool pq_update(void *_pq, int label, int cost)
{
   ...
   if ( pq  && pq->len < pq->size && label >= 0 && label < pq->size && pq->heapIDX[label] != -1 ) {
      ...
      int current = pq->heapIDX[label]; // get node pozition ( node for which we update cost)
      pq->cost[current] = cost; // update the cost
      int parent = GET_PARENT(current); // get parent node 
      while (current > 0 && pq->cost[parent] > pq->cost[current]) { // while cost of current node is less than cost of parent -> swap nodes
