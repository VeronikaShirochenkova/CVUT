...
_Bool dijkstra_load_graph(const char *filename, void *dijkstra)
{
   ...
   if ( dij && dij->graph && load_graph_simple(filename, dij->graph)) { // edges has not been loaded
...
_Bool dijkstra_set_graph(int e, int edges[][3], void *dijkstra)
{
   ...
   if (dij && dij->graph) {
      ...
      // same like dijkstra_load_graph
      int m = -1;
      for (int i = 0; i < dij->graph->num_edges; ++i) {
         const edge_t *const edge = &(dij->graph->edges[i]); // use pointer to avoid copying
         m = m < edge->from ? edge->from : m;
         m = m < edge->to ? edge->to : m;
      }
