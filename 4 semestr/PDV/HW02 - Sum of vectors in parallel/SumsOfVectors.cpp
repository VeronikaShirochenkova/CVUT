...
void sumsOfVectors_omp_shuffle(const vector<vector<int8_t>> &data, vector<long> &solution,
    ...
    std::vector<int> vec_of_idx(data.size());
    std::iota(vec_of_idx.begin(), vec_of_idx.end(), 0);
    std::random_shuffle(vec_of_idx.begin(), vec_of_idx.end());

#pragma omp parallel for schedule(static)
    for(unsigned long i = 0; i < vec_of_idx.size(); i++) {
        int index = vec_of_idx[i];
        long sum = 0;
        for (int8_t j : data[index]) {
            sum += j;
        }
        solution[index] = sum;
    }
